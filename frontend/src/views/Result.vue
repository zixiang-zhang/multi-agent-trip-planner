...
const addAttractionMarkers = (AMap: any) => {
  ...
  const dayColors = ['#4CAF50', '#1890ff', '#fa8c16', '#722ed1', '#eb2f96', '#13c2c2']
  ...
  allAttractions.forEach((attraction) => {
    const dayColor = dayColors[attraction.dayIndex % dayColors.length]
    const marker = new AMap.Marker({
      position: attraction.lnglat,
      title: attraction.name,
      label: {
        content: `<div style="background: ${dayColor}; color: white; padding: 4px 8px; border-radius: 4px; font-size: 12px; border: 2px solid #fff; box-shadow: 0 2px 8px rgba(0,0,0,0.2);">${attraction.daySequence}</div>`,
    ...
    const infoWindow = new AMap.InfoWindow({
      content: `
        <div style="padding: 10px;">
          ...
          <p style="margin: 4px 0; color: ${dayColor};"><strong>第${attraction.dayIndex + 1}天 第${attraction.daySequence}站</strong></p>
        </div>
      `,
      offset: new AMap.Pixel(0, -30)
    })
  ...
  drawRoutes(AMap, allAttractions, dayColors)
}

const drawRoutes = (AMap: any, attractions: any[], dayColors: string[]) => {
  ...
  Object.entries(dayGroups).forEach(([dayIndex, dayAttractions]: any) => {
    ...
    const polyline = new AMap.Polyline({
      path,
      strokeColor: dayColors[Number(dayIndex) % dayColors.length],
      strokeWeight: 4,
      strokeOpacity: 0.8,
      strokeStyle: 'solid',
      showDir: true
